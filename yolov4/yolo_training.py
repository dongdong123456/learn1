...
class YOLOLoss(nn.Module):
    def __init__(self, anchors, input_shape, cuda, anchors_mask=[[6, 7, 8], [3, 4, 5], [0, 1, 2]],
                 label_smoothing=0):
    ...
    def forward(self,input, targets=None):
        ...
        # -----------------------------------------------#
        prediction = input.view(bs, 3, self.bbox_attrs, in_h, in_w).permute(0, 1, 3, 4,
                                                                                                    2).contiguous()
        ...
        #   获得网络应该有的预测结果
        # -----------------------------------------------#
        y_true, noobj_mask, box_loss_scale = self.get_target(targets, scaled_anchors, in_h, in_w)
        ...
        #   作为负样本不合适
        # ----------------------------------------------------------------#
        noobj_mask, pred_boxes = self.get_ignore( x, y, h, w, targets, scaled_anchors, in_h, in_w, noobj_mask)
    ...
    def get_target(self, targets, anchors, in_h, in_w):
        # -----------------------------------------------------#
        #   计算一共有多少张图片
        # -----------------------------------------------------#
        bs = len(targets)
